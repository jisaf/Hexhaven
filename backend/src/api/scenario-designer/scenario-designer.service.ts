...
export class ScenarioDesignerService {
  private readonly scenariosPath = path.join(__dirname, '..', '..', 'data', 'scenarios.json');
  ...
  private async writeScenariosFile(data: { scenarios: Scenario[] }): Promise<void> {
  ...
  async findAll(): Promise<Omit<Scenario, 'mapLayout' | 'monsterGroups'>[]> {
    ...
    return scenarios.map(({ mapLayout, monsterGroups, ...rest }) => rest);
  }
